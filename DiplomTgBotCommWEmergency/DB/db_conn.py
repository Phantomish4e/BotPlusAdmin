...
def see_all_incidents():
    ...
    allincidents = cursor.execute('SELECT * FROM Incidents').fetchall()
    print(allincidents)
...
def get_dispatchers_tg_id():
    connection = sqlite3.connect('my_database.db')
    cursor = connection.cursor()

    dispatchers_tg_id = cursor.execute('SELECT user_id FROM Dispatchers').fetchall()
    dispatchers_tg_id = [int(entry[0]) for entry in dispatchers_tg_id]

    connection.close()

    return dispatchers_tg_id